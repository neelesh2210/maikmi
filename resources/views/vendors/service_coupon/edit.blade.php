...
<div class="card">
    <div class="card-body">
        <form action="{{ route('vendors.service-coupon.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select name="services_ids[]" id="services_ids" class="form-control js-example-basic-multiple" multiple data-placeholder="Select services" required>
                        @foreach ($servicesList as $service)
                            <option value="{{$service->id}}" @if(in_array($service->id, $data->services_ids)) selected @endif>{{$service->name}}</option>
                        @endforeach
                    </select>
                    @error('services_ids')
                        <small class="text-danger">{{ $message }}</small>
