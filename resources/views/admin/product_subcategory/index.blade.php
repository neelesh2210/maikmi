...
@section('content')
    <div class="row">
                            ...
                            <tbody>
                                @forelse ($list as $key => $data)
                                    <tr>
                                        ...
                                        <td>
                                            <x-edit-btn route="{{ route('product-subcategory.edit', $data->id) }}" />
                                            <x-delete-btn route="{{ route('product-subcategory.destroy', $data->id) }}" />
                                        </td>
                                    </tr>
                                @empty
        ...
        <div class="col-md-5">
            <div class="card">
                ...
                <div class="card-body">
                    @isset($edit_data)
                        <form id="valid_form" method="POST" action="{{route('product-subcategory.update',$edit_data->id)}}" enctype="multipart/form-data">
                        @method('PUT')
                    @else
                        <form id="valid_form" method="POST" action="{{route('product-subcategory.store')}}" enctype="multipart/form-data">
                    @endisset
                        @csrf
                        <div class="mb-3">
                            <label for="product_category_id" class="form-label">Product Category</label>
                            <select name="product_category_id" id="product_category_id" class="form-control js-example-basic-single">
                                <option value="">Select Product Category</option>
                                @foreach ($categoryList as $category)
                                    <option value="{{$category->id}}" @if(isset($edit_data) && $edit_data->product_category_id == $category->id) selected @endif>{{$category->name}}</option>
                                @endforeach
                            </select>
                        ...
                        @isset($edit_data)
                            <x-cancle-btn text="Cancel" route="{{route('product-subcategory.index')}}" />
                        @endisset
                    </form>
                </div>
            </div>
    ...
    <script>
        $(".featured_update").change(function(){
            var id = $(this).val();
            $.get("{{ route('product-subcategory.featureUpdate', '') }}/"+id, function(data)
            {
        ...
        $(".status_update").change(function(){
            var id = $(this).val();
            $.get("{{ route('product-subcategory.statusUpdate', '') }}/"+id, function(data)
            {
                const Toast = Swal.mixin({
                    toast: true,
                    position: 'top-end',
