...
@section('content')
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-end">
                            <x-add-btn route="{{route('service-coupon.create')}}" />
                        </div>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @forelse ($list as $key=> $data)
                                    <tr>
                                        ...
                                        <td>
                                            <x-edit-btn route="{{ route('service-coupon.edit', $data->id) }}" />
                                            <x-delete-btn route="{{ route('service-coupon.destroy', $data->id) }}" />
                                        </td>
                                    </tr>
                                @empty
    ...
    <script>
        $(".status_update").change(function(){
            var id = $(this).val();
            $.get("{{ route('service-coupon.statusUpdate', '') }}/"+id, function(data)
            {
